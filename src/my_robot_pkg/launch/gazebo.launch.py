...
def generate_launch_description():
    pkg_share = launch_ros.substitutions.FindPackageShare(package='my_robot_pkg').find('my_robot_pkg')
    default_model_path = os.path.join(pkg_share, 'urdf/my_robot.urdf')
    default_world_path = os.path.join(pkg_share, 'worlds/wavy_terrain.world')
    ...
    gazebo_process = launch.actions.ExecuteProcess(
        cmd=[
            ...
            LaunchConfiguration('world')  # Memuat file world
        ],
        output='screen'
    )
